...
def test_flatten_orders_layers_and_children(canvas_model):
    ...
    canvas_model.addItem({"type": "ellipse", "centerX": 0, "centerY": 0, "radiusX": 5, "radiusY": 5, "name": "B"})
    canvas_model.setParent(2, layer1.id)

    canvas_model.addLayer()  # Layer 2
    layer2 = canvas_model.getItems()[3]
    canvas_model.addItem({"type": "rectangle", "x": 10, "y": 0, "width": 10, "height": 10, "name": "C"})
...
def test_move_layer_keeps_children_grouped(canvas_model):
    ...
    canvas_model.addItem({"type": "rectangle", "x": 10, "y": 0, "width": 10, "height": 10, "name": "B"})
...
def test_reparent_moves_node_and_render_order_updates(canvas_model):
    ...
    canvas_model.addItem({"type": "rectangle", "x": 0, "y": 0, "width": 10, "height": 10, "name": "Rect"})
